...
public static class VertexTranformer{
    ...
    {
        Vector3[] verticeHolder = vertices;
        Vector3 lastVertex = vertices[0];
        float lastMagnitude = 0;
        bool tophightIsReached = false;
        for (int i = 0; i < vertices.Length; i++)
        {
            Vector3 hight = verticeHolder[i] + normal * (lastMagnitude + Random.Range(-peakIntensity, peakIntensity));

            if(tophightIsReached == true)
            {
                hight = verticeHolder[i] + normal * Random.Range(-peakIntensity, 0);
                tophightIsReached = false;
            }
            else if (Mathf.Abs(((Vector3)(vertices[i] - hight)).magnitude) >= topHeight) //See if 
            {
                hight = verticeHolder[i] + normal * topHeight;
                tophightIsReached = true;
            }
            else if (Mathf.Abs(((Vector3)(hight - lastVertex)).magnitude) < 0)
            {
                hight = verticeHolder[i];
            }
            lastMagnitude = Mathf.Abs(((Vector3)(verticeHolder[i] - hight)).magnitude);
            verticeHolder[i] = hight;
            lastVertex = vertices[i];
        }

        return verticeHolder;
    }
